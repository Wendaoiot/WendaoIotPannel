--[[
@module  main
@summary RS485 数据采集上报 demo —— 程序入口
@version 1.0

@usage
功能：RS485 总线(UART2, 9600 8N1, GPIO33 硬件方向控制)
  - 每 2 秒通过 485 发送一帧 ASCII 测试串（验证发送方向）
  - 485 收到的数据 → MQTT 主题 wendao/<IMEI>/rs485/up
  - 平台下发到 wendao/<IMEI>/rs485/down → 485 总线原样发出

文件说明：
  main.lua        本文件，入口（上电初始化 + 加载模块）
  mqtt_wendao.lua wendaoiot 平台 MQTT 连接（通用，几个 demo 共用同一文件）
  rs485_app.lua   RS485 收发业务
]]
...
-- 【38盒子 V3.1 板载电路】两路升压使能：EN1=GPIO37（RS485_5V 供电，本示例必需）、EN2=GPIO38
