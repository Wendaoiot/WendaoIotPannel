...
-- 挂起标志，休眠期间停止定时上报(配合sleep_test模块的低功耗休眠)
-- 周期性上报会不断唤醒系统，导致无法进入真正的低功耗休眠
...
sys.subscribe("APP_RESUME", function()
    if suspended then
        ...
        -- 恢复上报定时链
        sys.timerStart(send_data_req_timer_cbfunc, 1000)
        log.info("timer_app", "resumed, restart report chain")
    end
end)
...
function send_data_req_timer_cbfunc()
    -- 休眠挂起期间停止上报，定时链自然停止
    if suspended then
        log.info("timer_app", "suspended, stop report")
        return
    end
