...
local function network_watchdog_task_func()
    while true do
        -- 如果等待1200秒(20分钟)没有等到"FEED_NETWORK_WATCHDOG"消息，则看门狗超时
        -- 注意: sleep_test关闭定时唤醒后，休眠期间唯一的喂狗来源是
        -- MQTT keepalive(600s)的pong应答(mqtt_main收到pong时发布喂狗消息)，
        -- 看门狗超时必须大于keepalive周期，否则休眠中会被误重启
        if not sys.waitUntil("FEED_NETWORK_WATCHDOG", 1200000) then
