...
function mqtt_receiver.proc(topic, payload, metas)
    ...
    -- 任何MQTT下行消息都发布RECV_DATA_FROM_SERVER，用于远程唤醒
    -- 注意: 必须在topic-specific处理之前发布，否则control/ota等主题
    -- 在特定分支中return或不走else分支，导致sleep_test收不到唤醒信号
    ...
    if topic:match(control_topic_pattern) then
        ...
        -- 非控制/OTA主题: 数据已在函数开头发布RECV_DATA_FROM_SERVER
        -- 此处可添加透传数据的额外处理逻辑
    end
end
