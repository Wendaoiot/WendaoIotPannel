...
-- 打印设备ID（IMEI）到串口，方便识别设备
sys.timerStart(function()
    log.info("========================================")
    log.info("DEVICE ID:", mobile.imei())
    log.info("========================================")
end, 1000)
...
-- 网络初始化：打印网络状态
sys.timerStart(function()
    log.info("net", "IMEI", mobile.imei())
    log.info("net", "ICCID", mobile.iccid())
    log.info("net", "SimID", mobile.simid())
    log.info("net", "CSQ", mobile.csq())
    log.info("net", "Registered", mobile.status())
    -- log.info("net", "Operator", mobile.operator())  -- 固件不支持此函数
end, 3000)

-- 如果是专网卡或特殊运营商，取消以下注释设置APN
-- 格式：mobile.apn(卡槽, "APN名称", "用户名", "密码")
-- 示例：
-- 移动：mobile.apn(1, "CMNET", nil, nil)
-- 联通：mobile.apn(1, "3GNET", nil, nil)
-- 电信：mobile.apn(1, "CTNET", nil, nil)
-- mobile.flymode(0)
-- mobile.apn(1, "你的APN", "用户名", "密码")
